...
var xManualFactory func() Manual

// ManualFactory produces a Manual
func ManualFactory(factory func() Manual) {
	xManualFactory = factory
}

// GetManualGrainClient instantiates a new ManualGrainClient with given Identity
func GetManualGrainClient(c *cluster.Cluster, id string) *ManualGrainClient {
	if c == nil {
		panic(fmt.Errorf("nil cluster instance"))
	}
	if id == "" {
	...
	return &ManualGrainClient{Identity: id, cluster: c}
}

// GetManualKind instantiates a new cluster.Kind for Manual
func GetManualKind(opts ...actor.PropsOption) *cluster.Kind {
	props := actor.PropsFromProducer(func() actor.Actor {
		return &ManualActor{
			Timeout: 60 * time.Second,
		}
	}, opts...)
	kind := cluster.NewKind("Manual", props)
	return kind
}

// GetManualKind instantiates a new cluster.Kind for Manual
func NewManualKind(factory func() Manual, timeout time.Duration, opts ...actor.PropsOption) *cluster.Kind {
	xManualFactory = factory
	props := actor.PropsFromProducer(func() actor.Actor {
		return &ManualActor{
			Timeout: timeout,
...
// Manual interfaces the services available to the Manual
type Manual interface {
	Init(ctx cluster.GrainContext)
	Terminate(ctx cluster.GrainContext)
	ReceiveDefault(ctx cluster.GrainContext)
	Ping(req *PingRequest, ctx cluster.GrainContext) (*PingResponse, error)
}

// ManualGrainClient holds the base data for the ManualGrain
type ManualGrainClient struct {
	Identity string
	cluster  *cluster.Cluster
}

// Ping requests the execution on to the cluster with CallOptions
func (g *ManualGrainClient) Ping(r *PingRequest, opts ...cluster.GrainCallOption) (*PingResponse, error) {
	if g.cluster.Config.RequestLog {
		g.cluster.Logger().Info("Requesting", slog.String("identity", g.Identity), slog.String("kind", "Manual"), slog.String("method", "Ping"), slog.Any("request", r))
	}
	bytes, err := proto.Marshal(r)
	if err != nil {
		return nil, err
	...
	reqMsg := &cluster.GrainRequest{MethodIndex: 0, MessageData: bytes}
	resp, err := g.cluster.Request(g.Identity, "Manual", reqMsg, opts...)
	if err != nil {
		return nil, fmt.Errorf("error request: %w", err)
	}
	switch msg := resp.(type) {
...
// ManualActor represents the actor structure
type ManualActor struct {
	ctx     cluster.GrainContext
	inner   Manual
	Timeout time.Duration
}
...
// Receive ensures the lifecycle of the actor for the received message
func (a *ManualActor) Receive(ctx actor.Context) {
	switch msg := ctx.Message().(type) {
	case *actor.Started: //pass
	case *cluster.ClusterInit:
		a.ctx = cluster.NewGrainContext(ctx, msg.Identity, msg.Cluster)
		a.inner = xManualFactory()
		a.inner.Init(a.ctx)
	...
	case *cluster.GrainRequest:
		...
		case 0:
			...
			err := proto.Unmarshal(msg.MessageData, req)
			if err != nil {
				ctx.Logger().Error("[Grain] Ping(PingRequest) proto.Unmarshal failed.", slog.Any("error", err))
				resp := cluster.NewGrainErrorResponse(cluster.ErrorReason_INVALID_ARGUMENT, err.Error()).
					WithMetadata(map[string]string{
						"argument": req.String(),
					})
...
// onError should be used in ctx.ReenterAfter
// you can just return error in reenterable method for other errors
func (a *ManualActor) onError(err error) {
	resp := cluster.FromError(err)
	a.ctx.Respond(resp)
}
