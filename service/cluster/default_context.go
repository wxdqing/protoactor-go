...
func (dcc *DefaultContext) Request(placementContext *PlacementContext, identity, kind string, message interface{}, opts ...GrainCallOption) (interface{}, error) {
	var err error
...
func (dcc *DefaultContext) RequestFuture(placementContext *PlacementContext, identity string, kind string, message interface{}, opts ...GrainCallOption) (actor.Future, error) {
	var counter int
	callConfig := NewGrainCallOptions(dcc.cluster)
	for _, o := range opts {
		o(callConfig)
	...
	for {
		...
		default:
			...
			pid, _ := dcc.getPid(placementContext, identity, kind)
			if pid == nil {
				dcc.cluster.Logger().Debug("Requesting PID from IdentityLookup but got nil", slog.String("identity", identity), slog.String("kind", kind))
				counter = callConfig.RetryAction(counter)
				if dcc.cluster.metricsEnabled {
...
func (dcc *DefaultContext) getPid(placementContext *PlacementContext, identity, kind string) (*actor.PID, bool) {
	...
	if dcc.cluster.metricsEnabled {
		start := time.Now()
		pid = dcc.cluster.Get(placementContext, identity, kind)
		if pid != nil {
