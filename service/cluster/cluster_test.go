...
func (p *inmemoryProvider) Shutdown(_ bool) error {
...
func (l *fakeIdentityLookup) Get(_ *PlacementContext, identity *ClusterIdentity) *actor.PID {
...
func (l *fakeIdentityLookup) RemovePid(placementContext *PlacementContext, identity *ClusterIdentity, pid *actor.PID) {
	if existPid := l.Get(placementContext, identity); existPid.Equal(pid) {
		l.m.Delete(identity.Identity)
	}
}

func (l *fakeIdentityLookup) Setup(cluster *Cluster, _ []string, _ bool) {
	l.cluster = cluster
}

func (l *fakeIdentityLookup) Shutdown() {
}
...
func TestCluster_Call(t *testing.T) {
	...
	t.Run("invalid kind", func(_ *testing.T) {
		msg := struct{}{}
		resp, err := c.Request(nil, "name", "nonkind", &msg)
	...
	t.Run("normal", func(_ *testing.T) {
		msg := struct{ Code int }{9527}
		resp, err := c.Request(nil, "name", "kind", &msg)
		assert.NoError(err)
		assert.Equal(&struct{ Code int }{9528}, resp)
	})

	t.Run("timeout", func(_ *testing.T) {
		msg := struct{}{}
		resp, err := c.Request(nil, "name", "kind", &msg, WithTimeout(time.Millisecond))
...
func TestCluster_Get(t *testing.T) {
	...
	t.Run("invalid kind", func(t *testing.T) {
		...
		pid := c.Get(nil, "name", "nonkind")
		assert.Nil(pid)
	})

	t.Run("ok", func(t *testing.T) {
		assert := assert.New(t)
		pid := c.Get(nil, "name", "kind")
		assert.NotNil(pid)
	})
}
